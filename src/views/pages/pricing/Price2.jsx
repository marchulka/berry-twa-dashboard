...
const FeatureTitleWrapper = styled(CardContent)(({ theme }) => ({
  background:
    theme.palette.mode === ThemeMode.DARK ? `${theme.palette.background.default} !important` : `${theme.palette.grey[100]} !important`,
  textAlign: 'left',
  paddingTop: 12,
...
const FeatureContentWrapper = styled(CardContent)(({ theme }) => ({
  ...
  borderColor:
    theme.palette.mode === ThemeMode.DARK ? `${theme.palette.background.default} !important` : `${theme.palette.grey[200]} !important`,
  position: 'relative',
  overflow: 'hidden',
...
export default function Price2() {
  ...
  return (
    <Grid container spacing={gridSpacing}>
      <Grid size={12}>
        <Alert variant="outlined" severity="info" sx={{ borderColor: 'info.main' }}>
          ...
          <Typography>
            ...
            <Link
              sx={{ textDecoration: 'none', ml: 0.5, ...(themeDirection === ThemeDirection.RTL && { mr: 0.5, ml: 0 }) }}
              variant="subtitle1"
              target="_blank"
      ...
      <Grid size={12}>
        <Stack direction="row" spacing={gridSpacing} sx={{ alignItems: 'center', justifyContent: 'center' }}>
